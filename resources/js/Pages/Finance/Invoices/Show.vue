            ...
            <div class="grid gap-4 lg:grid-cols-2">
                <Card title="Totals" rule="total = received + credited + outstanding">
                    <dl class="grid grid-cols-2 gap-2 text-sm">
                        <div><dt class="text-xs text-ink-500">Subtotal</dt><dd class="font-medium tnum">{{ money(invoice.subtotal, invoice.currency) }}</dd></div>
                        ...
                        <div><dt class="text-xs text-ink-500">Credited</dt><dd class="font-medium tnum text-amber-700">{{ money(invoice.credited ?? 0, invoice.currency) }}</dd></div>
                ...
                <Card v-if="creditNotes.length" title="Credit notes" :padded="false">
                    <ul class="divide-y divide-slate-100 text-sm">
                        <li v-for="note in creditNotes" :key="note.id" class="flex items-center justify-between gap-2 px-4 py-2">
                            ...
                            <span class="tnum">{{ money(note.amount, invoice.currency) }}</span>
                ...
                <Card title="Receipts against this invoice" :padded="false">
                    <ul class="divide-y divide-slate-100 text-sm">
                        <li v-for="(allocation, index) in allocations" :key="index" class="flex items-center justify-between px-4 py-2">
                            ...
                            <span class="tnum">{{ money(allocation.amount, invoice.currency) }}</span>
                        </li>
