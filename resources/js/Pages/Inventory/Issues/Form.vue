                ...
                <Card title="Issue" rule="BR-37">
                    <dl class="space-y-2.5 text-sm">
                        ...
                        <div class="flex items-baseline justify-between gap-3 border-t border-slate-100 pt-2.5">
                            <dt class="text-xs text-ink-500">Value issued</dt>
                            <dd class="text-base font-semibold tnum text-ink-900">{{ money(totalValue) }}</dd>
                        </div>
                    </dl>

                    <!-- Breaking FIFO is allowed; breaking it silently is not, so the block sits
                         where the eye goes before pressing post. -->
                    <p
                        v-if="missingReason"
