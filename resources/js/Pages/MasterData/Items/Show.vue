        ...
        <div class="grid gap-4 lg:grid-cols-3">
            <Card title="Master data" class="lg:col-span-1">
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between"><dt class="text-slate-500">Standard rate</dt><dd class="tnum">{{ money(item.std_rate) }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Weighted average</dt><dd class="tnum font-medium">{{ money(item.avg_rate) }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Reorder level</dt><dd class="tnum">{{ qty(item.reorder_level) }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Min order / multiple</dt><dd class="tnum">{{ qty(item.min_order_qty) }} / {{ qty(item.order_multiple) }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Safety days</dt><dd class="tnum">{{ item.safety_days }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Ink lay g/m²</dt><dd class="tnum">{{ item.ink_lay_gsm ?? '—' }}</dd></div>
                ...
                <ul class="divide-y divide-slate-100 text-sm">
                    <li v-for="row in stock" :key="row.warehouse_code" class="flex items-center justify-between py-2">
                        <span>
                            ...
                            <span class="text-slate-500"> {{ row.warehouse_name }}</span>
                            <Badge v-if="!row.is_nettable" tone="neutral" label="non-net" class="ml-1" />
                        </span>
                        <span class="tnum font-medium">{{ qty(row.balance_qty) }}</span>
                    </li>
                    <li v-if="stock.length === 0" class="py-6 text-center text-slate-500">No stock on hand.</li>
                </ul>
            </Card>
