        ...
        <div class="grid gap-4 lg:grid-cols-3">
            <Card title="Capability and cost" rule="BR-16 · BR-18 · BR-27">
                <dl class="space-y-2 text-sm">
                    <div class="flex justify-between"><dt class="text-slate-500">Standard rate / hour</dt><dd class="tnum">{{ machine.std_rate_per_hour ? qty(machine.std_rate_per_hour, 0) : '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Cost / hour</dt><dd class="tnum">{{ money(machine.hourly_rate) }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">kW rating</dt><dd class="tnum">{{ machine.kw_rating ?? '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Efficiency</dt><dd class="tnum">{{ machine.efficiency_pct }}%</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Web width</dt><dd class="tnum">{{ machine.web_width_mm ?? '—' }} mm</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Max colours</dt><dd class="tnum">{{ machine.max_colours ?? '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Status</dt><dd><Badge :status="machine.status" /></dd></div>
                </dl>
            </Card>
