...
<template>
    <AppLayout>
        ...
        <div class="space-y-4">
            ...
            >
                <template #actions>
                    <span class="text-sm font-semibold tnum text-slate-900">{{ money(line.line_total) }}</span>
                ...
                <div v-if="line.cost_sheet" class="grid gap-0 lg:grid-cols-3">
                        ...
                        <table class="min-w-full text-xs">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="cl in line.cost_lines" :key="cl.sequence_no">
                                    <td class="px-3 py-1.5 font-medium text-slate-800">{{ titleCase(cl.cost_type) }}</td>
                                    <td class="px-3 py-1.5 text-slate-500">{{ cl.basis_uom }}</td>
                                    ...
                                    <td class="px-3 py-1.5">
                                        <span v-if="cl.formula_ref" class="rounded bg-slate-100 px-1 font-mono text-[10px] text-slate-600">
                                            {{ cl.formula_ref }}
                                        </span>
                                    </td>
                                </tr>
                    ...
                    <div class="border-t border-slate-200 p-3 text-sm lg:border-t-0 lg:border-l">
                        <dl class="space-y-1.5">
                            <div class="flex justify-between"><dt class="text-slate-500">Gross metres</dt><dd class="tnum">{{ qty(line.cost_sheet.gross_metres) }}</dd></div>
                            <div class="flex justify-between"><dt class="text-slate-500">Total wastage</dt><dd class="tnum">{{ Number(line.cost_sheet.total_wastage_pct).toFixed(2) }}%</dd></div>
                            <div class="flex justify-between"><dt class="text-slate-500">Material</dt><dd class="tnum">{{ money(line.cost_sheet.material_cost) }}</dd></div>
                            <div class="flex justify-between"><dt class="text-slate-500">Machine + labour + energy</dt><dd class="tnum">{{ money(Number(line.cost_sheet.machine_cost) + Number(line.cost_sheet.labour_cost) + Number(line.cost_sheet.energy_cost)) }}</dd></div>
                            <div class="flex justify-between"><dt class="text-slate-500">Overheads</dt><dd class="tnum">{{ money(line.cost_sheet.overhead_amount) }}</dd></div>
                            ...
                            <div class="flex justify-between"><dt class="text-slate-500">Unit cost</dt><dd class="tnum">{{ Number(line.cost_sheet.unit_cost).toFixed(6) }}</dd></div>
                            <div class="flex justify-between">
                                <dt class="text-slate-500">Margin <span class="font-mono text-[10px] text-slate-400">BR-20</span></dt>
            ...
            <Card title="Document total">
                <dl class="flex flex-wrap gap-8 text-sm">
                    <div><dt class="text-slate-500">Subtotal</dt><dd class="text-lg font-semibold tnum">{{ money(quotation.subtotal) }}</dd></div>
                    <div><dt class="text-slate-500">Tax</dt><dd class="text-lg font-semibold tnum">{{ money(quotation.tax_amount) }}</dd></div>
                    <div><dt class="text-slate-500">Total</dt><dd class="text-lg font-semibold tnum text-brand-800">{{ money(quotation.total) }}</dd></div>
                    <div><dt class="text-slate-500">Valid until</dt><dd class="text-lg font-semibold">{{ date(quotation.valid_until) }}</dd></div>
                </dl>
            </Card>
        </div>
        ...
        <Modal v-model:open="convertOpen" title="Convert to a sales order" subtitle="Q3: only an accepted quotation converts.">
            <div class="space-y-3">
                ...
                <FormField label="Delivery date" :error="convertForm.errors.delivery_date">
                    <TextInput v-model="convertForm.delivery_date" type="date" />
                </FormField>
            </div>
            <template #footer="{ close }">
                <Button @click="close">Cancel</Button>
